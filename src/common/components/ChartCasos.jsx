...
class ChartCasos extends Component {
    ...
    getDados() {
        let chart;
        const request = axios.get(`${consts.API_URL}/dashboard/casosAcumulados`)
            .then(request => {
                const obt = [2,1,4,0,1 ]
                const data = [{
                    label: request.data.descricao,
                    data: request.data.acumulados,
                    fill: false,
